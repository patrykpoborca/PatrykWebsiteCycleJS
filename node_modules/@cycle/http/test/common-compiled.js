...
function run(uri) {
  ...
  describe('HTTP Driver', function () {
    it('should throw when request stream emits neither string nor object', function (done) {
      ...
      response$$.mergeAll().subscribe(function onNext() {
        assert.fail();
      }, function onError(err) {
        assert.strictEqual(err.message, 'Observable of requests given to ' + 'HTTP Driver must emit either URL strings or objects with ' + 'parameters.');
        done();
      });
    });
    ...
    it('should send 500 server errors to response$ onError', function (done) {
      ...
      response$$.subscribe(function (response$) {
        ...
        response$.subscribe(function onNext() {
          assert.fail();
        }, function onError(err) {
